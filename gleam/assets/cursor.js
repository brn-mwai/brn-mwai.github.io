(() => {
  ...
  const draw = () => {
    ...
    cursor.style.setProperty("--tilt", `${tilt}deg`);
    cursor.style.setProperty("--look", `${look}px`);
  };
  const schedule = () => {
  ...
  const blink = () => {
    ...
    setTimeout(() => cursor.classList.remove("is-blinking"), 120);
  };
  const blinkLater = () => {
    setTimeout(() => {
      if (!document.hidden && cursor.classList.contains("is-on")) {
        blink();
        if (Math.random() < 0.2) setTimeout(blink, 260);
      }
      blinkLater();
  ...
  window.addEventListener("pointermove", (event) => {
    ...
    lastX = x;
    if (!still) {
      tilt = Math.max(-14, Math.min(14, moved * 0.9));
      look = Math.max(-16, Math.min(16, moved * 2));
    }
    cursor.classList.add("is-on");
    cursor.classList.toggle("is-pointing", Boolean(event.target.closest?.(interactive)));
    clearTimeout(settle);
    settle = setTimeout(() => {
      tilt = 0;
      look = 0;
